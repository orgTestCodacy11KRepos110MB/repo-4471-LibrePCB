...
#include "drc/boardclipperpathgenerator.h"
...
BoardPreviewPainter::BoardPreviewPainter(const Board& board)
        ...
        board.getDefaultFontName()))  // can throw
{
  ...
  // Generate board outline polygons.
  //{
  //  BoardClipperPathGenerator gen(board, maxArcTolerance);
  //  gen.addBoardOutline();
  //  gen.addHoles(Length(0));
  //  std::unique_ptr<ClipperLib::PolyTree> area = ClipperHelpers::area(gen.getPaths());  // can throw
  //  ClipperLib::Paths areaPaths = ClipperHelpers::flattenTree(*area);  // can throw
  //  foreach (const ClipperLib::Path& path, areaPaths) {
  //    mPolygons.append(polygon->getPolygon());
  //  }
  //}
}

BoardPreviewPainter::~BoardPreviewPainter() noexcept {
}
...
void BoardPreviewPainter::paint(QPainter& painter,
...
void BoardPreviewPainter::initContentByLayer() const noexcept {
  ...
  if (mContentByLayer.isEmpty()) {
    ...
    foreach (const Footprint& footprint, mFootprints) {
      ...
      foreach (Polygon polygon, footprint.polygons) {
        ...
        if (polygon.getLayerName() == GraphicsLayer::sBoardOutlines) {
          boardOutline.addPath(polygon.getPath().toQPainterPathPx());
        } else {
      ...
      foreach (Circle circle, footprint.circles) {
        ...
        if (circle.getLayerName() == GraphicsLayer::sBoardOutlines) {
          const qreal r = circle.getDiameter()->toPx() / 2;
          boardOutline.addEllipse(circle.getCenter().toPxQPointF(), r, r);
        } else {
      ...
      foreach (Hole hole, footprint.holes) {
        ...
        const qreal r = hole.getDiameter()->toPx() / 2;
        boardOutline.addEllipse(hole.getPosition().toPxQPointF(), r, r);
      }

      // Footprint pads.
      foreach (FootprintPad pad, footprint.pads) {
        const qreal r = pad.getDrillDiameter()->toPx() / 2;
        QPainterPath path = footprint.transform
                                .map(pad.getOutline()
        ...
        path.addEllipse(
            footprint.transform.map(pad.getPosition()).toPxQPointF(),
            r,
            r);
        boardOutline.addEllipse(
            footprint.transform.map(pad.getPosition()).toPxQPointF(),
              r,
              r);
        QString layer;
        ...
        layer = footprint.transform.map(layer);
    ...
    foreach (const Via& via, mVias) {
      const qreal r = via.getDrillDiameter()->toPx() / 2;
      boardOutline.addEllipse(
          via.getPosition().toPxQPointF(),
            r,
            r);
    ...
    // Board area.
    mContentByLayer[GraphicsLayer::sBoardOutlines].areas.append(boardOutline);
  }
}
